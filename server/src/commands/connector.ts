...
import {
  generateAuthUrl as googleGenerateAuthUrl,
  ...
  switch (sub) {
    ...
    case 'auth': {
      ...
      if (service === 'm365') {
        ...
        auth = googleGenerateAuthUrl(clientId, redirectUri, scopes);
      } else {
      ...
      return new Promise<number>((resolve) => {
        const server = createServer((req, res) => {
          ...
          const exchangePromise = service === 'm365'
            ? m365ExchangeCode(code, pkce.verifier, pkce.clientId, pkce.clientSecret, pkce.redirectUri, (cfg.tenant as string) ?? 'common')
            : service === 'slack'
              ? slackExchangeCode(code, pkce.clientId, pkce.clientSecret, pkce.redirectUri)
              : googleExchangeCode(code, pkce.verifier, pkce.clientId, pkce.clientSecret, pkce.redirectUri);
          ...
          exchangePromise
            .then(async (tokens) => {
              ...
              if (tokenAny.refresh_token) {
                newState.refresh_token = tokenAny.refresh_token;
              }
