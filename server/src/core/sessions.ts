 ...
 *
 * Provides a thin wrapper over the OIDC provider's session store,
 * with additional features:
 *   - Session creation with tenant identity mapping
 *   - Session validation with expiry checking
 *   - Session refresh (extend expiry)
 *   - Session invalidation (logout)
 *   - Tenant-scoped session listing (for audit)
 *   - Session token generation (opaque, random)
 *
 * WP-209 scope:
 ...
 *   - Single-use state tokens (no replay)
...
const SESSION_REFRESH_THRESHOLD_MS = 60 * 60 * 1000; // Refresh if < 1h left
...
class SessionManager {
  ...
  /**
   * Validate a session by ID. Returns session info if valid.
   * Automatically refreshes sessions close to expiry.
   */
  ...
  /**
   * Get all active sessions for a tenant (admin/audit).
   */
  ...
  /**
   * Record a login_started audit event.
   */
