...
describe('M365 OAuth Helper', () => {
  test('generateAuthUrl produces valid URL with PKCE', async () => {
    ...
    const result = generateAuthUrl('test-client-id', 'http://localhost:3000/oauth/callback', M365_DEFAULT_SCOPES, 'common');
  ...
  test('generateAuthUrl with specific tenant', async () => {
    ...
    const result = generateAuthUrl('cid', 'http://localhost:3000/callback', 'Files.Read.All', '11111111-2222-3333-4444-555555555555');
  ...
  test('exchangeCode throws on non-ok response', async () => {
    ...
    globalThis.fetch = mock(() => Promise.resolve(new Response('{"error":"invalid_grant"}', { status: 400 }))) as unknown as typeof fetch;
    try {
      await expect(exchangeCode('code', 'verifier', 'cid', 'secret', 'http://localhost:3000/callback', 'common')).rejects.toThrow('M365 token exchange failed');
  ...
  test('refreshAccessToken throws on non-ok response', async () => {
    ...
    globalThis.fetch = mock(() => Promise.resolve(new Response('{"error":"invalid_grant"}', { status: 400 }))) as unknown as typeof fetch;
    try {
      await expect(refreshAccessToken('rt', 'cid', 'secret', 'common')).rejects.toThrow('M365 token refresh failed');
    } finally { globalThis.fetch = originalFetch; }
  });
  ...
  test('exchangeCode sends correct parameters', async () => {
    ...
    globalThis.fetch = mock((_url: string | URL | Request, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({ access_token: 'at', refresh_token: 'rt', expires_in: 3600, token_type: 'Bearer', scope: 'Files.Read.All' }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
...
describe('M365Connector', () => {
  ...
  test('constructs with specific tenant and sources', async () => {
    const { M365Connector } = await import('../src/core/ingestion/connectors/m365.ts');
    const connector = new M365Connector({ client_id: 'cid', client_secret: 'cs', tenant: 'my-tenant-id', sources: ['files'] });
    expect(connector.id).toBe('m365');
  });
  ...
  test('fetchDelta fetches files, mail, and calendar with mocked Graph API', async () => {
    const { M365Connector } = await import('../src/core/ingestion/connectors/m365.ts');
    const connector = new M365Connector({ client_id: 'cid', client_secret: 'cs' });
    (connector as any)._state = { connector_id: 'm365', service: 'm365', access_token: 'test-at', refresh_token: 'test-rt', token_expires_at: Date.now() + 3600_000 };
    ...
    globalThis.fetch = mock((url: string | URL | Request) => {
      ...
      if (urlStr.includes('/me/drive/root/delta')) {
        return Promise.resolve(new Response(JSON.stringify({
          value: [
            { id: 'file-1', name: 'document.docx', lastModifiedDateTime: '2026-01-01T00:00:00Z', webUrl: 'https://my.sharepoint.com/document.docx', file: { mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }, parentReference: { driveId: 'drive-1' } },
            { id: 'folder-1', name: 'My Folder', lastModifiedDateTime: '2026-01-01T00:00:00Z', folder: { childCount: 5 } },
          ],
      ...
      if (urlStr.includes('/me/messages')) {
        return Promise.resolve(new Response(JSON.stringify({
          value: [{ id: 'msg-1', subject: 'Test Email', body: { content: '<p>Hello World</p>', contentType: 'HTML' }, receivedDateTime: '2026-01-01T10:00:00Z', webLink: 'https://outlook.live.com/mail/1', from: { emailAddress: { address: 'test@example.com', name: 'Test User' } } }],
        }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      }
      if (urlStr.includes('/me/events')) {
        return Promise.resolve(new Response(JSON.stringify({
          value: [{ id: 'evt-1', subject: 'Team Meeting', body: { content: 'Weekly sync', contentType: 'Text' }, start: { dateTime: '2026-01-05T09:00:00Z' }, end: { dateTime: '2026-01-05T10:00:00Z' }, webLink: 'https://outlook.live.com/calendar/1', location: { displayName: 'Conference Room A' } }],
        }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      }
  ...
  test('fetchDelta with sources=files only fetches files', async () => {
    const { M365Connector } = await import('../src/core/ingestion/connectors/m365.ts');
    const connector = new M365Connector({ client_id: 'cid', client_secret: 'cs', sources: ['files'] });
    (connector as any)._state = { connector_id: 'm365', service: 'm365', access_token: 'test-at', refresh_token: 'test-rt', token_expires_at: Date.now() + 3600_000 };
    ...
    globalThis.fetch = mock((url: string | URL | Request) => {
      ...
      const urlStr = url.toString();
      if (urlStr.includes('/me/drive/root/delta')) {
        return Promise.resolve(new Response(JSON.stringify({ value: [{ id: 'f1', name: 'test.txt', lastModifiedDateTime: '2026-01-01T00:00:00Z', file: { mimeType: 'text/plain' } }], '@odata.deltaLink': 'https://graph.microsoft.com/v1.0/me/drive/root/delta?token=xyz' }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      }
  ...
  test('fetchDelta handles Graph API error', async () => {
    const { M365Connector } = await import('../src/core/ingestion/connectors/m365.ts');
    const connector = new M365Connector({ client_id: 'cid', client_secret: 'cs', sources: ['files'] });
    (connector as any)._state = { connector_id: 'm365', service: 'm365', access_token: 'test-at', token_expires_at: Date.now() + 3600_000 };

    const originalFetch = globalThis.fetch;
    globalThis.fetch = mock(() => Promise.resolve(new Response('{"error":{"code":"TokenExpired"}}', { status: 401 }))) as unknown as typeof fetch;
  ...
  test('toIngestionEvent produces correct event shape for mail', async () => {
    ...
    const event = await connector.toIngestionEvent({
      id: 'msg-1', title: 'Test Email', modified_at: '2026-01-01T10:00:00Z',
      content: '<p>Hello</p>', content_type: 'text/html',
  ...
  test('composite cursor round-trips correctly', async () => {
    const { M365Connector } = await import('../src/core/ingestion/connectors/m365.ts');
    const connector = new M365Connector({ client_id: 'cid', client_secret: 'cs', sources: ['files', 'mail'] });
    (connector as any)._state = { connector_id: 'm365', service: 'm365', access_token: 'test-at', token_expires_at: Date.now() + 3600_000 };
    ...
    globalThis.fetch = mock((url: string | URL | Request) => {
      ...
      if (urlStr.includes('/me/drive/root/delta')) {
        filesUrl = urlStr;
        return Promise.resolve(new Response(JSON.stringify({ value: [], '@odata.deltaLink': 'https://graph.microsoft.com/v1.0/me/drive/root/delta?token=files-token' }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      }
      if (urlStr.includes('/me/messages')) {
    ...
    try {
      ...
      await connector.fetchDelta(result1.nextCursor);
