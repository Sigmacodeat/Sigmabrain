...
// Taxumio — dedicated, agency-grade landing page for tax & accounting firms.
// Sells the daily firm workflow (Kanzleialltag) and links to the real
// dashboard tools that ship today. Animated cockpit mockup, day timeline,
...
function Cockpit({ c }: { c: (typeof TAXUMIO)["en"]["cockpit"] }) {
  return (
    <motion.div
          ...
          >
            ...
            <ul className="space-y-1.5">
              {c.digestItems.map((it, i) => (
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${it.includes("!") || it.includes("today") || it.includes("heute") ? "bg-rose-400" : "bg-amber-400"}`} />
                  {it}
                </motion.li>
              ))}
            </ul>
...
export default function TaxumioPage({ lang }: { lang: Lang }) {
  const t = TAXUMIO[lang];
  const signupHref = p(lang, "/signup?industry=tax");
