...
class PgApiKeyStore implements ApiKeyStore {
  ...
  private row(r: Record<string, unknown>): StoredApiKey {
    return {
      ...
      scopes: Array.isArray(r.scopes) ? r.scopes as string[] : JSON.parse(String(r.scopes ?? "[]")),
  ...
  async create(key: StoredApiKey) {
    ...
    await this.pool().query(
      ...
      [
        key.id, key.ownerId, key.name, key.prefix, key.secretHash,
        JSON.stringify(key.scopes), key.active, key.createdAt, key.createdBy,
      ],
    );
    return key;
  }
  ...
  async update(id: string, patch: Partial<StoredApiKey>) {
    ...
    if (patch.scopes !== undefined) { sets.push(`scopes = $${i++}`); vals.push(JSON.stringify(patch.scopes)); }
    if (patch.lastUsedAt !== undefined) { sets.push(`last_used_at = $${i++}`); vals.push(patch.lastUsedAt); }
