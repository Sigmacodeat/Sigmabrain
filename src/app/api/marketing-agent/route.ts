...
const INDUSTRY_WORDS: Record<string, string[]> = {
  ...
  tax: ["tax", "accounting", "steuer", "datev", "gobd", "taxumio", "wp"],
  compliance: ["compliance", "grc", "aml", "dsgvo", "gdpr", "ai act", "compliumio"],
  insurance: ["insurance", "broker", "makler", "policy", "police", "schaden", "versumio"],
  realestate: ["real estate", "property", "immobil", "miete", "lease", "immumio"],
  vc: ["vc", "private equity", "fund", "investor", "dealflow", "founder", "investumio"],
  consulting: ["consulting", "consultant", "consultants", "advisor", "advisors", "agency", "beratung", "berater", "agentur", "pitch", "consultumio"],
  recruiting: ["recruiting", "search", "talent", "kandidat", "candidate", "talentumio"],
  medical: ["medical", "arzt", "praxis", "patient", "medumio"],
};
...
function chipsFor(lang: Lang, fields: Partial<Record<FieldKey, string>>, industry: string | null): string[] {
  if (!industry) return lang === "de"
    ? ["Kanzlei", "Steuerberatung", "Compliance", "Versicherung", "VC", "Recruiting"]
    : ["Law firm", "Tax & accounting", "Compliance", "Insurance", "VC", "Recruiting"];
...
function answerProductQuestion(lang: Lang, text: string, industry: string | null): string | null {
  ...
  if (/(compare|better|besser|unterschied|chatgpt|notion|glean|dms|drive)/.test(lower)) {
    return lang === "de"
      ? "Der Unterschied: Chatbots beantworten Prompts, DMS/Drive speichern Dateien. Sigmabrain baut ein dauerhaftes Brain über eure eigenen Daten: Hybrid-Suche, Graph, belegte Synthese und explizite Lücken."
      : "The difference: chatbots answer prompts, DMS/Drive store files. Sigmabrain builds a durable brain over your own data: hybrid search, graph, cited synthesis and explicit gaps.";
  }
  if (profile && /(what|was|kann|does|product|produkt|lösung|solution)/.test(lower)) {
    return lang === "de"
      ? `${profile.brand} ist Sigmabrain für ${profile.label.de}: ${profile.signature.proof.de}`
      : `${profile.brand} is Sigmabrain for ${profile.label.en}: ${profile.signature.proof.en}`;
  }
  return null;
}
...
export async function POST(req: NextRequest) {
  ...
  const ctaHref = plan === "enterprise"
    ? "mailto:hello@sigmabrain.com?subject=Sigmabrain%20Enterprise"
  ...
  const intro = productAnswer ?? (lang === "de"
    ? profile
      ...
      : "Ich kann dich produktübergreifend beraten: Sigmabrain, Subsumio, Taxumio, Compliumio, Versumio, Immumio, Investumio, Consultumio und Talentumio."
    : profile
      ? `${profile.brand} looks like a good fit for ${profile.label.en}. I’ll qualify the setup so you don’t land in the wrong plan.`
      : "I can advise across Sigmabrain, Subsumio, Taxumio, Compliumio, Versumio, Immumio, Investumio, Consultumio and Talentumio.");
