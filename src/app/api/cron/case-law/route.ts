...
async function filterNewHits(brainId: string, hits: JudgementHit[]): Promise<JudgementHit[]> {
  ...
  if (!pool) return hits; // Dev ohne DB: kein Dedup
  await pool.query(`
    CREATE TABLE IF NOT EXISTS sigmabrain_caselaw_seen (
  ...
  `);
  const fresh: JudgementHit[] = [];
  for (const hit of hits) {
    const { rowCount } = await pool.query(
      `INSERT INTO sigmabrain_caselaw_seen (brain_id, hit_id) VALUES ($1, $2)
       ON CONFLICT (brain_id, hit_id) DO NOTHING`,
      [brainId, hit.id],
    );
    if (rowCount && rowCount > 0) fresh.push(hit);
  }
  return fresh;
}
...
export async function GET(req: NextRequest) {
  ...
  // Brain → Empfänger (Org-Mitglieder teilen das Org-Brain).
