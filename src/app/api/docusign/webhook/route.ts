...
export async function POST(req: NextRequest) {
  ...
  // Update Brain-Page: search for page with docusign_envelope_id = envelopeId
  // across all known brains. In production, iterate tenants or use a DB index.
  const brainId = process.env.GBRAIN_BRAIN || "default";
  let updated = false;
  try {
  ...
  if (eventId) markWebhookProcessed(eventId);
