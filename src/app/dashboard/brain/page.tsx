  ...
  return (
    <div className="flex h-full overflow-hidden">
      <div className="w-52 shrink-0 border-r border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] p-4 space-y-1 overflow-y-auto">
        <p className="text-xs text-[color:var(--ds-text-muted)] uppercase tracking-wider font-medium mb-3">Typ</p>
        {TYPE_FILTERS.map((f) => {
          const Icon = f.icon;
          return (
            <button
        ...
        <div className="pt-4 pb-2">
          <p className="text-xs text-[color:var(--ds-text-muted)] uppercase tracking-wider font-medium mb-3">Sortierung</p>
          {[
        ...
        <div className="px-6 py-4">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <BookOpen size={40} className="text-[color:var(--ds-border)] mb-4" />
              <h3 className="text-lg font-semibold text-[color:var(--ds-text)] mb-2">Brain ist leer</h3>
              <p className="text-sm text-[color:var(--ds-text-muted)] mb-6 max-w-sm">
              ...
              {displayed.map((page) => {
                ...
                return (
                  <a
                    ...
                    className="flex items-center gap-4 p-4 rounded-xl border border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] hover:border-[color:var(--ds-border-strong)] hover:bg-[color:var(--ds-hover)] transition-all group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {"snippet" in page && page.snippet && (
                        <p className="text-xs text-[color:var(--ds-text-muted)] mt-2 line-clamp-2">{page.snippet}</p>
                      )}
                      {tags && tags.length > 0 && (
                        <div className="flex items-center gap-1 mt-2">
                          {tags.map((tag) => (
                            <span key={tag} className="text-xs font-mono text-[color:var(--ds-text-muted)] bg-[color:var(--ds-border)] px-1.5 py-0.5 rounded">
                              #{tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <ChevronRight size={16} className="text-[color:var(--ds-text-muted)] group-hover:text-violet-600 transition-colors shrink-0" />
                  </a>
                );
              })}
            </div>
