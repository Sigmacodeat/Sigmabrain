  ...
  return (
    <div className="relative flex items-center">
      <input
        ...
        className="w-full bg-[#0d0d1a] border border-[#1e1e3a] rounded-lg px-3 py-2.5 pr-20 text-sm text-[#e8e8f0] placeholder:text-[#8a8aa8] focus:outline-none focus:border-violet-500/50 font-mono transition-colors"
      />
      <div className="absolute right-2 flex items-center gap-1">
        <button
          onClick={() => setShow(!show)}
          className="p-1.5 text-[#8a8aa8] hover:text-[#8888aa] transition-colors"
        >
          {show ? <EyeOff size={13} /> : <Eye size={13} />}
        </button>
        {value && (
          <button onClick={copy} className="p-1.5 text-[#8a8aa8] hover:text-[#8888aa] transition-colors">
  ...
  return (
    <div className="grid grid-cols-3 gap-4 items-start py-4 border-b border-[#1e1e3a] last:border-0">
      <div>
        ...
        {desc && <p className="text-xs text-[#8a8aa8] mt-0.5 leading-relaxed">{desc}</p>}
      </div>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
                ...
                {[
                  ...
                  <div key={cmd} className="flex items-center gap-2 bg-[#0a0a18] border border-[#1e1e3a] rounded-lg px-4 py-2.5">
                    ...
                    <button
                      ...
                      className="text-[#8a8aa8] hover:text-[#8888aa] transition-colors shrink-0"
                    >
                      <Copy size={12} />
                    </button>
                  </div>
      ...
      {activeTab === "kanzlei" && (
        <Card>
          <div className="p-6 border-b border-[#1e1e3a]">
            <h2 className="text-base font-semibold text-[#e8e8f0]">Kanzlei-Einstellungen</h2>
            <p className="text-sm text-[#8888aa] mt-1">Verrechnung, Stundensatz & Abrechnung</p>
          </div>
          <div className="px-6 divide-y divide-[#1e1e3a]">
            ...
            <Field label="Kanzlei-Adresse" desc="Absender und Rechnungskopf">
              <textarea
                ...
                rows={3}
                className="w-full bg-[#0d0d1a] border border-[#1e1e3a] rounded-lg px-3 py-2.5 text-sm text-[#e8e8f0] placeholder:text-[#8a8aa8] focus:outline-none focus:border-violet-500/50"
              />
            </Field>
                  ...
                  <div className="space-y-2">
                    {Object.entries(rechtsgebietSaetze).map(([gebiet, satz]) => (
                      <div key={gebiet} className="flex items-center gap-3">
                        <span className="text-sm text-[#8888aa] w-32 capitalize">{gebiet}</span>
                        <Euro size={12} className="text-[#8a8aa8]" />
                        <input
                          type="number"
                          value={String(satz)}
                          onChange={(e) => {
                          ...
                          className="w-24 bg-[#0d0d1a] border border-[#1e1e3a] rounded-lg px-3 py-1.5 text-sm text-[#e8e8f0] focus:outline-none focus:border-violet-500/50"
                        />
                        <span className="text-xs text-[#8a8aa8]">€/h</span>
                      </div>
                    ))}
                  </div>
                </Field>
            ...
            <Field label="Zahlungsziel" desc="Standard-Fälligkeit neuer Rechnungen">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-[#8888aa]">Tage netto</span>
              </div>
            </Field>
      ...
      {activeTab === "team" && (
        <Card>
          <div className="p-6 border-b border-[#1e1e3a]">
            <h2 className="text-base font-semibold text-[#e8e8f0]">Team</h2>
            <p className="text-sm text-[#8888aa] mt-1">Kanzlei-Mitarbeiter und Rollen</p>
          </div>
          <div className="px-6 divide-y divide-[#1e1e3a]">
            {teamMembers.length === 0 ? (
              <div className="py-8 text-center text-sm text-[#8888aa]">Keine Team-Mitglieder gefunden.</div>
            ) : (
              teamMembers.map((member) => (
                <div key={member.id} className="py-4 flex items-center justify-between gap-4">
                  <div>
                    <div className="text-sm font-medium text-[#e8e8f0]">{member.name}</div>
                    <div className="text-xs text-[#8a8aa8]">{member.email}</div>
                  </div>
                  <select
                    value={member.role}
                    onChange={async (e) => {
                    ...
                    className="bg-[#0d0d1a] border border-[#1e1e3a] rounded-lg px-3 py-1.5 text-sm text-[#e8e8f0] focus:outline-none focus:border-violet-500/50"
                  >
