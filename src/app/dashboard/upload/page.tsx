...
export default function UploadPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[color:var(--ds-text)]">Dokument hochladen</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-[color:var(--ds-text-muted)] uppercase tracking-wider font-medium mb-2">
            Brain Source
          </label>
          <select
            value={source}
            onChange={(e) => setSource(e.target.value)}
            className="w-full bg-[color:var(--ds-surface)] border border-[color:var(--ds-border)] rounded-lg px-3 py-2.5 text-sm text-[color:var(--ds-text)] focus:outline-none focus:border-violet-500/50 transition-colors"
          >
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-[color:var(--ds-surface)] border border-[color:var(--ds-border)] rounded-lg px-3 py-2.5 text-sm text-[color:var(--ds-text)] placeholder:text-[color:var(--ds-text-muted)] focus:outline-none focus:border-violet-500/50 transition-colors"
          />
        </div>
      </div>
      ...
      <label className="flex items-start gap-3 p-4 rounded-xl border border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] cursor-pointer hover:border-[color:var(--ds-border-strong)] transition-colors">
        ...
        <span className="flex items-start gap-2.5 text-sm">
          <Archive size={15} className="text-violet-600 shrink-0 mt-0.5" />
      ...
      <div
        ...
        className={cn(
          ...
          isDragActive
            ...
            : "border-[color:var(--ds-border)] hover:border-violet-500/30 hover:bg-violet-500/[0.02]",
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className={cn(
            ...
            isDragActive ? "bg-violet-500/20" : "bg-[color:var(--ds-border)]"
          )}>
            <CloudUpload size={28} className={isDragActive ? "text-violet-600" : "text-[color:var(--ds-text-muted)]"} />
          </div>
          <div>
            ...
            <p className="text-sm text-[color:var(--ds-text-muted)]">
              oder <span className="text-violet-600 hover:underline">Dateien auswählen</span>
            </p>
          </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl border border-blue-500/20 bg-blue-500/5">
        ...
        <div className="text-sm text-[color:var(--ds-text-muted)] leading-relaxed">
          <strong className="text-[color:var(--ds-text)]">Wie funktioniert es?</strong> Sigmabrain chunked das Dokument automatisch,
          ...
          <br />
          <strong className="text-blue-600 mt-1 block">Hinweis:</strong> Die Sigmabrain Engine muss laufen ({`gbrain serve`}).
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {files.map((f) => (
              <div
                key={f.id}
                className="flex items-center gap-3 p-4 rounded-xl border border-[color:var(--ds-border)] bg-[color:var(--ds-surface)]"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {f.status === "done" && f.slug && (
                    <span className="flex items-center gap-2 flex-wrap">
                      ...
                      {f.gobdStamped && (
                        <Badge variant="default" className="text-[10px] bg-violet-500/10 text-violet-600 border border-violet-500/20 gap-1">
                ...
                <div className="shrink-0">
                  ...
                  {f.status === "uploading" && (
                    <Loader size={14} className="text-violet-600 animate-spin" />
                  )}
                  {f.status === "done" && (
