    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#e8e8f0]">Fristen & Termine</h1>
            <p className="text-sm text-[#8888aa]">{deadlines.length} Fristen aus allen Akten</p>
          </div>
        </div>
      ...
      {showCalc && (
        <div className="rounded-xl border border-[#1e1e3a] bg-[#0d0d1a] p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-[#e8e8f0]">Fristenberechnung</h2>
            <button onClick={() => setShowCalc(false)} className="text-[#8a8aa8] hover:text-[#e8e8f0]"><XCircle size={16} /></button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-[#8888aa] mb-1">Frist-Typ</label>
              <div className="relative">
                <select
                  value={calcTemplate.key}
                  onChange={(e) => setCalcTemplate(DEADLINE_RULES.find((t) => t.key === e.target.value) || DEADLINE_RULES[0])}
                  className="w-full bg-[#0a0a18] border border-[#1e1e3a] rounded-lg px-3 py-2 text-sm text-[#e8e8f0] focus:outline-none focus:border-violet-500/50 appearance-none"
                >
                  {DEADLINE_RULES.map((t) => (
                    <option key={t.key} value={t.key}>{t.label} ({t.law})</option>
                  ))}
                </select>
                <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8a8aa8] pointer-events-none" />
              </div>
              <p className="text-[11px] text-[#8a8aa8] mt-1">{calcTemplate.description}</p>
            </div>
            <div>
              <label className="block text-xs text-[#8888aa] mb-1">Startdatum (Zustellung / Ereignis)</label>
              <input
                type="date"
                value={calcDate}
            ...
            <div className="rounded-lg border border-violet-500/20 bg-violet-500/5 p-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-[#8a8aa8] mt-1">{calcResult.note}</p>
                </div>
        ...
        <div className="rounded-xl border border-[#1e1e3a] bg-[#0d0d1a] p-4 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-[#e8e8f0]">Fristen erkennen</h2>
            </div>
            <button onClick={() => setShowAiDetect(false)} className="text-[#8a8aa8] hover:text-[#e8e8f0]"><XCircle size={16} /></button>
          </div>
          ...
          <div className="flex gap-2">
            <textarea
              ...
              rows={4}
              className="flex-1 bg-[#0a0a18] border border-[#1e1e3a] rounded-lg px-3 py-2 text-sm text-[#e8e8f0] placeholder:text-[#8a8aa8] focus:outline-none focus:border-violet-500/50 resize-none"
            />
          </div>
          <button
            onClick={async () => {
          ...
          {aiResults.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-xs font-semibold text-[#e8e8f0]">{aiResults.length} Frist(en) erkannt</h3>
              {aiResults.map((r, i) => (
                <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-[#0a0a18] border border-[#1e1e3a]">
                  <div className={`w-2 h-2 rounded-full ${r.confidence === "high" ? "bg-emerald-400" : r.confidence === "medium" ? "bg-amber-400" : "bg-red-400"}`} />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-[#e8e8f0]">{r.description}</div>
                    {r.date && <div className="text-xs text-[#8a8aa8]">{new Date(r.date).toLocaleDateString("de-DE")}</div>}
                  </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 space-y-4">
          <CalendarClock size={48} className="mx-auto text-[#1e1e3a]" />
          <div>
            <p className="text-[#8888aa] text-lg">Keine Fristen gefunden</p>
          ...
          {filtered.map((d) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-[#e8e8f0]">{d.description}</span>
                ...
                <div className="text-right shrink-0">
                  <div className={cn("text-sm font-medium", days < 0 ? "text-red-400" : days <= 3 ? "text-amber-400" : "text-[#e8e8f0]")}>
