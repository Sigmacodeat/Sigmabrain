  ...
  return (
    <div className="p-6 max-w-full space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#e8e8f0]">Massen-Review</h1>
          <p className="text-sm text-[#8888aa]">
            Eine Frage gegen viele Dokumente — Antworten im Raster, jede Zelle mit Quelle
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-[#1e1e3a] bg-[#0d0d1a] p-4 space-y-4">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="block text-xs text-[#8888aa] mb-1">Dokumenttyp (Zeilen)</label>
            <select
              value={docType}
          ...
          <div>
            <label className="block text-xs text-[#8888aa] mb-1">Max. Dokumente</label>
            <input
              type="number" min={1} max={50} value={limit}
              onChange={(e) => setLimit(Math.min(Math.max(1, Number(e.target.value) || 1), 50))}
              className="w-24 bg-[#0a0a18] border border-[#1e1e3a] rounded-lg px-3 py-2 text-sm text-[#e8e8f0] focus:outline-none focus:border-violet-500/50"
            />
          </div>
        </div>

        <div className="space-y-2">
          <label className="block text-xs text-[#8888aa]">Fragen (Spalten, max. 8)</label>
          {questions.map((q, i) => (
            <div key={i} className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-[#0a0a18] border border-[#1e1e3a] rounded-lg px-3 py-2 text-sm text-[#e8e8f0] focus:outline-none focus:border-violet-500/50"
              />
              {questions.length > 1 && (
                <button onClick={() => removeQuestion(i)} className="text-[#8a8aa8] hover:text-red-400 p-2"><X size={15} /></button>
              )}
            </div>
          ))}
          {questions.length < 8 && (
        ...
        <Button onClick={run} disabled={loading} className="gap-2 bg-violet-600 hover:bg-violet-500 text-white">
      ...
      {result && result.rows.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="overflow-x-auto rounded-xl border border-[#1e1e3a]">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-[#12122a]">
                  <th className="text-left px-4 py-3 text-[#e8e8f0] font-semibold sticky left-0 bg-[#12122a] min-w-[200px]">Dokument</th>
                  {result.questions.map((q, i) => (
                    <th key={i} className="text-left px-4 py-3 text-[#c8c8e0] font-medium min-w-[240px] border-l border-[#1e1e3a]">{q}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {result.rows.map((row) => (
                  <tr key={row.slug} className="border-t border-[#1e1e3a] hover:bg-[#0d0d1a]/50">
                    <td className="px-4 py-3 text-[#e8e8f0] align-top sticky left-0 bg-[#0a0a18] font-medium">
                      <a href={`/dashboard/brain/${row.slug}`} className="hover:text-violet-300">{row.title}</a>
                    </td>
                    {row.cells.map((cell, i) => (
                      <td key={i} className="px-4 py-3 text-[#8888aa] align-top border-l border-[#1e1e3a]/60 leading-relaxed">
                        {cell.answer}
